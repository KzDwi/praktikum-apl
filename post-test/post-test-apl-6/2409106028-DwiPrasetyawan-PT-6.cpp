...
void hapusKendaraan(Kendaraan *kendaraan) { // parameter dereference
...
// Bubble sort, Gunakan Huruf, Secara Descending, Variabel Pemilik Kendaraan
void BubbleSort(Kendaraan parkiran[], int size) {
    for (int i = 0; i < size - 1; i++) {
        for (int j = 0; j < size - i - 1; j++) {
            if (parkiran[j].pemilik < parkiran[j + 1].pemilik) { // Mengurutkan berdasarkan pemilik
                Kendaraan temp = parkiran[j];
...
int AmbilNomor(const string& platNomor) { //Digunakan untuk Merge Sort karena harus memisahkan angka dari huruf
    string numberStr;
    for (char c : platNomor) {
        if (isdigit(c)) {
            numberStr += c;
        }
    }
    return numberStr.empty() ? 0 : stoi(numberStr); // Kembalikan 0 jika tidak ada angka, stoi untuk mengubah ke integer ('s'tring 'to' 'i'nteger)
}
// Merge Sort, Gunakan Angka, Secara Ascending, Variabel Plat Nomor Kendaraan
void merge(Kendaraan arr[], int left, int mid, int right) {
    int i, j, k;
    int n1 = mid - left + 1; // sub_array kiri
    int n2 = right - mid;    // sub_array kanan
    
    // Array static untuk sub-array
    Kendaraan L[MAX_X];
    Kendaraan R[MAX_X];
    
    // Menyalin data ke sub-array L dan R
    for (i = 0; i < n1; i++)
    L[i] = arr[left + i];
    for (j = 0; j < n2; j++)
    R[j] = arr[mid + 1 + j];
    ...
    j = 0; 
    k = left; 
    // Gabung sub-array L dan R
    while (i < n1 && j < n2) {
        if (AmbilNomor(L[i].platNomor) <= AmbilNomor(R[j].platNomor)) {
            arr[k] = L[i];
            i++;
        } else {
            arr[k] = R[j];
    ...
    // simpan sisa elemen dari L jika ada
    while (i < n1) {
        arr[k] = L[i];
        i++;
        k++;
    }
    // simpan sisa elemen dari R jika ada
    while (j < n2) {
        arr[k] = R[j];
        j++;
        k++;
...
void mergeSort(Kendaraan arr[], int left, int right) {
    if (left < right) {
...
// Insertion Sort, Gunakan Huruf, Secara Ascending, Variabel Pemilik Kendaran
void InsertionSort(Kendaraan parkiran[], int size) {
    for (int i = 1; i < size; i++) {
        Kendaraan key = parkiran[i];
        int j = i - 1;
        
        while (j >= 0 && parkiran[j].pemilik > key.pemilik) { // Urutkan berdasarkan pemilik
            parkiran[j + 1] = parkiran[j];
            j = j - 1;
        }
        parkiran[j + 1] = key;
    }
    ...
    do {
        ...
        switch (pilihan) {
            ...
            case 5: {
                ...
                cout << "\n==================================" << endl;
                cout << "1. Sorting Huruf Pemilik (Bubble, Descending)\n";
                cout << "2. Sorting Angka Plat Nomor (Merge, Ascending)\n";
                cout << "3. Sorting Huruf Pemilik (Insertion, Ascending)\n";
                cout << "\n==================================" << endl;
                cout << "Pilihan: ";
                cin >> sortChoice;
                ...
                if (sortChoice == 1) {
                    for (int i = 0; i < MAX_Y; i++) {
                        BubbleSort(parkiranMobil[i], MAX_X);
                    }
                    cout << "Sorting huruf selesai.\n";
                } else if (sortChoice == 2) {
                    for (int i = 0; i < MAX_Y; i++) {
                        mergeSort(parkiranMobil[i], 0, MAX_X - 1);
                    }
                    cout << "Sorting angka selesai.\n";
                } else if (sortChoice == 3) {
                    for (int i = 0; i < MAX_Y; i++) {
                        InsertionSort(parkiranMobil[i], MAX_X);
                    }
                    cout << "Sorting pemilik selesai.\n";
                } else {
